...
const BrandIcon: React.FC<BrandIconProps> = ({ brand = 'generic', size = 16, className = '', title, src }) => {
  const s = size;
  const common = { width: s, height: s, viewBox: '0 0 16 16', xmlns: 'http://www.w3.org/2000/svg', preserveAspectRatio: 'xMidYMid meet' } as const;
  const fixedBox: React.CSSProperties = { width: s, height: s, minWidth: s, minHeight: s, display: 'block', flex: '0 0 auto' };
  ...
  if (src) {
    ...
    return (
      <span
        className={className}
        style={{ width: s, height: s, minWidth: s, minHeight: s, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flex: '0 0 auto' }}
  ...
  if (brand === 'gemini') {
    ...
    return (
      <svg {...common} className={className} aria-hidden={!title} role={title ? 'img' : 'presentation'} style={fixedBox}>
  ...
  if (brand === 'gpt') {
    ...
    return (
      <svg {...common} className={className} aria-hidden={!title} role={title ? 'img' : 'presentation'} style={fixedBox}>
        {title && <title>{title}</title>}
        <path fill="#374151" d="M8 1.6c1.1-.6 2.5-.5 3.6.2 1.1.7 1.7 1.9 1.7 3.1v.3c.9.6 1.5 1.6 1.5 2.7 0 1.2-.6 2.2-1.6 2.8-.1 1.2-.8 2.3-1.9 2.9-1.1.6-2.5.5-3.5-.2-1 .7-2.4.8-3.5.2S1.7 11.8 1.6 10.6C.6 10 .1 8.9.1 7.7c0-1.1.6-2.1 1.5-2.7.1-1.2.8-2.3 1.9-3C4.5 1.4 6 .3 8 1.6Zm0 1.7c-1-.6-2.1-.5-3 .1-.9.6-1.4 1.6-1.3 2.6l-.8.5c-.7.4-1.1 1.1-1.1 1.9 0 .8.4 1.5 1.1 1.9l.7.4v.8c0 1 .5 2 1.4 2.6.9.6 2 .6 2.9 0l.7-.4.7.4c.9.6 2 .6 2.9 0 .9-.6 1.4-1.6 1.4-2.6v-.8l.7-.4c.7-.4 1.1-1.1 1.1-1.9 0-.8-.4-1.5-1.1-1.9l-.8-.5v-.8c0-1-.5-2-1.3-2.6-.9-.6-2-.6-3-.1l-.7.4-.7-.4Zm2.6 2.6 1.3.8-1.3.8v1.6L9.3 10l-1.3-.8-1.3.8L5.4 9.5V7.9L4.1 7.1l1.3-.8V4.7l1.3-.8 1.3.8 1.3-.8 1.3.8v1.6Z"/>
      </svg>
    );
  }
  if (brand === 'claude') {
    ...
    return (
      <svg {...common} className={className} aria-hidden={!title} role={title ? 'img' : 'presentation'} style={fixedBox}>
        ...
        <g stroke="#F59E0B" strokeWidth="1.2" vectorEffect="non-scaling-stroke">
