    ...
    <div className="p-3 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <label className="text-sm">显示名
          <input className="mt-1 w-full p-1.5 bg-transparent border-0 outline-none focus:ring-0" value={role.displayName} onChange={e => onChange({ ...role, displayName: e.target.value })} />
        </label>
        <label className="text-sm">渠道
          <select className="mt-1 w-full p-1.5 bg-transparent border-0 outline-none focus:ring-0" value={role.providerId} onChange={e => onChange({ ...role, providerId: e.target.value })}>
        ...
        {!hideUserPromptTemplate && (
          <label className="text-sm col-span-2">User 模板
            <textarea className="mt-1 w-full p-1.5 bg-transparent border-0 outline-none focus:ring-0 h-20" value={role.userPromptTemplate || ''} onChange={e => onChange({ ...role, userPromptTemplate: e.target.value })} />
          </label>
        )}
        ...
        <div className="col-span-2 grid grid-cols-2 gap-3">
          <div className="text-sm p-2">
            <div className="flex items-center justify-between">
              ...
              <label className="inline-flex items-center gap-1">
                <input type="checkbox" checked={params.temperature !== undefined} onChange={e => setParams({ temperature: e.target.checked ? (params.temperature ?? 1) : undefined })} />
              </label>
            </div>
          ...
          <div className="text-sm p-2">
            <div className="flex items-center justify-between">
              <span>Top‑P (0–1)</span>
              <label className="inline-flex items-center gap-1">
                <input type="checkbox" checked={params.top_p !== undefined} onChange={e => setParams({ top_p: e.target.checked ? (params.top_p ?? 1) : undefined })} />
              </label>
            </div>
          ...
          <div className="text-sm p-2">
            <div className="flex items-center justify-between">
              ...
              <label className="inline-flex items-center gap-1">
                <input type="checkbox" checked={params.reasoning_effort !== undefined} onChange={e => setParams({ reasoning_effort: e.target.checked ? (params.reasoning_effort ?? 'medium') : undefined })} />
              </label>
            </div>
            <select disabled={params.reasoning_effort === undefined} className="mt-2 w-full p-1.5 bg-transparent border-0 outline-none focus:ring-0" value={params.reasoning_effort ?? 'medium'} onChange={e => setParams({ reasoning_effort: e.target.value as any })}>
          ...
          <div className="text-sm p-2">
            <div className="flex items-center justify-between">
              <span>verbosity</span>
              <label className="inline-flex items-center gap-1">
                <input type="checkbox" checked={params.verbosity !== undefined} onChange={e => setParams({ verbosity: e.target.checked ? (params.verbosity ?? 'medium') : undefined })} />
              </label>
            </div>
            <select disabled={params.verbosity === undefined} className="mt-2 w-full p-1.5 bg-transparent border-0 outline-none focus:ring-0" value={params.verbosity ?? 'medium'} onChange={e => setParams({ verbosity: e.target.value as any })}>
              <option value="low">low</option>
              <option value="medium">medium</option>
              <option value="high">high</option>
            </select>
        ...
        {availableVariablesText && (
          <div className="col-span-2">
            <pre className="text-xs bg-gray-50 p-2 overflow-auto">{availableVariablesText}</pre>
          </div>
        )}
      </div>
    </div>
